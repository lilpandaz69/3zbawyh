...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  try {
    if ($act === 'create') {
      ...
      if ($username==='' || $password==='') throw new Exception('أدخل اسم مستخدم وباسورد');
      // دور
      ...
      $st = $pdo->prepare("INSERT INTO users(username,password_hash,role_id,is_active) VALUES (?,?,?,1)");
      $st->execute([$username,$hash,$role_id]);
...
// قراءة القائمة
$rows = $pdo->query("SELECT u.id, u.username, u.is_active, r.name AS role_name, u.created_at
...
<head>
  ...
  <style>.muted{color:#666}.badge{padding:2px 6px;border-radius:6px;background:#eee}</style>
</head>
<body>
<div class="container">
  <nav class="nav">
    ...
    <ul>
      ...
      <li><a href="/3zbawyh/public/pos.php">POS</a></li>
  ...
  <div class="card" style="margin-top:12px">
    ...
    <table class="table">
      <thead><tr><th>#</th><th>المستخدم</th><th>الدور</th><th>الحالة</th><th>أُنشئ في</th><th>إجراءات</th></tr></thead>
      <tbody>
        ...
        <tr>
          ...
          <td style="display:flex;gap:6px">
            ...
            <form method="post" onsubmit="return confirm('تأكيد إعادة الباسورد؟')">
              ...
              <input type="hidden" name="id" value="<?=e($r['id'])?>">
              <input class="input" name="newpw" placeholder="باسورد جديد" style="width:150px">
